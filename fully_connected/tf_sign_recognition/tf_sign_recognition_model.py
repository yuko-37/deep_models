...
def load_datasets():
    with h5py.File('datasets/train_signs.h5') as train_ds:
        train_x = train_ds['train_set_x'][:]
        train_y = train_ds['train_set_y'][:]

    with h5py.File('datasets/test_signs.h5') as test_ds:
...
def train_model(mini_batch_size=32, learning_rate=0.0001, num_epochs=100, printCost=True):
...
if __name__ == '__main__':
    parameters, costs, train_acc, test_acc = train_model(num_epochs=100)
